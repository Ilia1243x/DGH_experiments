...
from grad_cond import gradient_descent, Adam
...
def upper(X, Y, c='auto', iter_budget=100, S0=None, tol=1e-16, return_fg=False,
          lb=0, validate_tri_ineq=False, verbose=0, rnd=None, cfind = False):
    """
    ...
    if c == 'auto':
        ...
        for c_test in C_SEARCH_GRID:
            ...
            ub, f, g = upper(X, Y, c=c_test, iter_budget=search_iter_budget_per_c,
                             S0=S0, tol=tol, return_fg=True, lb=lb, rnd=rnd, cfind = True)
            if ub < best_dis_R/2:
                c = c_test
    ...
    while iter_budget > 0:
        ...
        if (cfind == False and S0 is not None):
            print(iter_budget, used_iter,'S0',c)
        ...
        #S, used_iter = gradient_descent(S_prev = S0, c = c, X = X, Y = Y, max_iter = iter_budget)
        S, used_iter = Adam(S_prev = S0, c = c, X = X, Y = Y, max_iter = iter_budget)
        if (cfind == False):
            print (iter_budget, used_iter,c)
        ...
        if abs(best_dis_R - lb) <= 0.5:
            if (cfind == False):
                print(iter_budget + used_iter, 'break',c)
            break
